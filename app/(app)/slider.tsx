import { Canvas, Circle, Group, Path, usePathValue, vec, LinearGradient, RadialGradient, Shadow, Paint, Blur, Mask, } from "@shopify/react-native-skia";
...
const Knob = memo(({ knobX, knobY, isPressed, knobSize }: KnobProps) => {
	// Create a derived value for scale with smooth transition
	const scale = useDerivedValue(() => {
		return withTiming(isPressed.value ? 1.05 : 1, {
			duration: 100,
		});
	});
	...
	return (
		...
		>
			...
			<Circle cx={0} cy={0} r={knobSize} style="stroke" strokeWidth={3}>
				<LinearGradient start={vec(0, knobSize)} end={vec(0, -knobSize)} colors={["#BFBFBF", "#fff"]} />
			</Circle>

			{/* Inner Filled Circle */}
			<Circle cx={0} cy={0} r={knobSize - 1} style="fill">
				<LinearGradient start={vec(0, knobSize)} end={vec(0, knobSize)} colors={["#D5D5D5", "#fff"]} />
			</Circle>
		</Group>
	);
});
...
export default function Page() {
	...
	const gesture = useMemo(() => {
		return Gesture.Pan()
			...
			.onUpdate((e) => {
				...
				const touchX = e.x - CENTER;
				const touchY = e.y - CENTER;
			...
			.onEnd(() => {
				...
				const nearestMonth = Math.round(progressReanimated.value * MAX_MONTHS) / MAX_MONTHS;
	...
	const startCapPath = usePathValue((path) => {
		...
		// Bottom-right corner
		path.quadTo(capX + capWidth, capY + capHeight, capX + capWidth - cornerRadius, capY + capHeight);
		// Bottom edge
	...
	const mainGroupTransform = useMemo(() => {
		// No need for translation since canvas is already CIRCLE_SIZE
		return [];
	}, []);
				...
				<View ref={canvasRef} style={styles.canvasContainer}>
					<Canvas style={styles.canvas}>
						<Group transform={mainGroupTransform}>
							...
							<Circle cx={CENTER} cy={CENTER} r={CIRCLE_RADIUS - CIRCLE_STROKE_WIDTH / 2} color="#F7F7F7">
							...
							<Group>
								...
								>
									<Group>
										<Circle cx={knobX} cy={knobY} r={(INNER_CIRCLE_STROKE_WIDTH / 2) * 2.5} color="#FF385C">
											<Blur blur={40} />
										</Circle>
									</Group>
								</Mask>
							...
							>
								...
								>
									...
									{/* End cap circle */}
									<Circle cx={knobX} cy={knobY} r={INNER_CIRCLE_STROKE_WIDTH / 2} color="#FF385C" style="fill">
										<RadialGradient
											c={vec(CENTER, CENTER)}
											r={CIRCLE_SIZE / 2}
											colors={PROGRESS_GRADIENT_COLORS}
							...
							<Knob knobX={knobX} knobY={knobY} isPressed={isPressed} knobSize={KNOB_SIZE} />
						</Group>
					</Canvas>
				</View>
			</GestureDetector>
